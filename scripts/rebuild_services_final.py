...
with open('c:/Users/my/Desktop/chatgpt/scripts/services_backup.html', 'r', encoding='utf-8') as f:
    backup_html = f.read()
...
for block in dept_blocks:
    ...
    dept_name = header.get_text(strip=True)
    
    desc = block.find('p')
    desc_text = desc.get_text(strip=True) if desc else ""
    
    services = []
    cards = block.find_all('a', class_='shadcn-service-card')
    for card in cards:
        href = card.get('href')
        title_el = card.find('h3', class_='shadcn-card-title')
        title = title_el.get_text(strip=True) if title_el else ""
        
        desc_el = card.find('p', class_='shadcn-card-desc')
        service_desc = desc_el.get_text(strip=True) if desc_el else ""
