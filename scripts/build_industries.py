...
# Read a base template from an existing service page to keep the exact same HTML shell
with open('c:/Users/my/Desktop/chatgpt/services/web-applications.html', 'r', encoding='utf-8', errors='ignore') as f:
    template = f.read()

# We need to extract the nav, the CTA, and the footer from the template, and swap the middle content.
...
'''
    ...
    # Fix paths in nav/footer since these are in a subfolder (like services)
    # The template came from services/ web-applications.html, so paths are ALREADY relative to a subfolder!
...
print("Generated 6 high-value industry pages.")
